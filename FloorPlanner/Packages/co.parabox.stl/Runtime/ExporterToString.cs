...
{
	public static class ExporterToString
	{
		/// <summary>
		...
		/// <returns></returns>
		public static string ExportToString(string path, GameObject[] gameObjects, FileType type)
		{
			...
			if (meshes != null && meshes.Length > 0)
			{
				//if (!string.IsNullOrEmpty(path))
					content = ExporterToString.WriteFile(path, meshes, type);
			}
		...
		/// <returns></returns>
		public static string WriteFile(string path, Mesh mesh, FileType type = FileType.Ascii, bool convertToRightHandedCoordinates = true)
		{
		 ...
		 */
		public static string WriteFile(string path, IList<Mesh> meshes, FileType type = FileType.Ascii, bool convertToRightHandedCoordinates = true)
		{
			string model = "";
			try
			{
		...
		public static string WriteString(IList<Mesh> meshes, bool convertToRightHandedCoordinates = true)
		{
			...
			foreach (Mesh mesh in meshes)
			{
				...
				for (int i = 0; i < triLen; i += 3)
				{
					...
					sb += string.Format("facet normal {0} {1} {2}", nrm.x, nrm.y, nrm.z) + "\\n";
					//sb.AppendLine("outer loop");
					sb += "outer loop\\n";
					//sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[a].x, v[a].y, v[a].z));
					sb += string.Format("\\tvertex {0} {1} {2}", v[a].x, v[a].y, v[a].z) + "\\n";
					//sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[b].x, v[b].y, v[b].z));
					sb += string.Format("\\tvertex {0} {1} {2}", v[b].x, v[b].y, v[b].z) + "\\n";
					//sb.AppendLine(string.Format("\tvertex {0} {1} {2}", v[c].x, v[c].y, v[c].z));
					sb += string.Format("\\tvertex {0} {1} {2}", v[c].x, v[c].y, v[c].z) + "\\n";
			...
			sb += string.Format("endsolid {0}", name) + "\\n";
			return sb;
		}
