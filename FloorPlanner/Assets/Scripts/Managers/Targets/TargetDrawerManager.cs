using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class TargetDrawerManager : MonoBehaviour
{
    private void Awake()
    {
        var modelFactory = new TargetDrawerModelFactory();
        var model = modelFactory.Model;
        var model2 = modelFactory.Model2;
        var model3 = modelFactory.Model3;

        var viewFactory = new TargetDrawerViewFactory();
        var view = viewFactory.View;
        var view2 = viewFactory.View2;
        var view3 = viewFactory.View3;
        var view4 = viewFactory.View4;

        var controllerFactory = new TargetDrawerControllerFactory(model, model2, model3, view, view2, view3, view4);
        var controller = controllerFactory.Controller;


    }
}
